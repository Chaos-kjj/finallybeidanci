...
状态（2026-08-05）：Bigme B7 Pro 真机待连接。canonical debug APK 已通过 `npm run build` 和 `npm run android:install` 安装到 `emulator-5554`；模拟器只能作为软件回归证据，不能替代下方硬件项目。

## 已完成的 Android 软件/模拟器证据
...
本次安装后首页截图：`/tmp/beidanci-final-20260805-installed-home.png`。构建指纹与 APK 新鲜度以最后一次 `npm run build` 输出为准。2026-08-04 的真实 EPUB/PDF 回归属于历史证据，本次未重新导入仓库外样本。
...
- Bigme B7 Pro 真机未连接：实体键码、墨水屏刷新、厂商 WebView/SAF、横竖屏和长时间运行未通过。
- 仓库没有 release keystore：当前 APK 只适合 debug 侧载，不是正式发布签名包。
- 未提供新的真实 EPUB/PDF/StarDict/MDX/MDD 样本；特殊压缩、加密和非标准资源仍按风险项处理。
