(function (root, factory) {
    ...
    const PDF_LAYOUT_FIELDS = Object.freeze([
        'zoom', 'fitWidth', 'rotate', 'contrast', 'crop', 'invert', 'grayscale'
    ]);

    function normalizeFormat(format) {
        const value = String(format || '').toLowerCase();
        if (value === 'epub' || value === 'pdf' || value === 'html' || value === 'text') return value;
        return 'text';
    }
    ...
    function settingVisibility(format, flow) {
        ...
        return {
            ...
            typography: normalizedFormat !== 'pdf' || !pdfOriginal,
            pdfLayout: normalizedFormat === 'pdf' && pdfOriginal,
            pdfFlow: normalizedFormat === 'pdf',
            reflow: !pdfOriginal
        };
    }
    ...
    function bindReaderMenu({ closeButton, settingsButton, menu, body, settingsPanel, tocPanel, titleElement, getTitle, onClose } = {}) {
        ...
        settingsButton?.addEventListener('click', () => {
            toggleReaderPanel(settingsPanel, null, tocPanel);
        });
