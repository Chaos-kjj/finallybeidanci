(function (root, factory) {
    ...
    function formatLabel(type) {
        return { epub: 'EPUB', pdf: 'PDF', html: 'HTML', markdown: 'Markdown', text: 'TXT' }[normalizeFormat(type)] || 'TXT';
    }
    ...
    function deterministicTextCover({ title = '未命名书籍', author = '', type = 'text' } = {}) {
        ...
        const colors = { epub: ['#111', '#f4f0e6'], pdf: ['#111', '#e9edf2'], html: ['#111', '#f0eee9'], markdown: ['#111', '#f5f5f5'], text: ['#111', '#fff'] };
    ...
    function normalizeBookRecord(book = {}, { now = Date.now() } = {}) {
        ...
        const title = String(book.title || book.fileName || '未命名书籍').replace(/\.(?:epub|pdf|txt|md|markdown|html?|mht)$/i, '') || '未命名书籍';
        const updatedAt = Number(book.updatedAt) || now;
        const lastReadAt = book.lastReadAt || book.progress?.updatedAt || (book.progress?.percent > 0 ? new Date(updatedAt).toISOString() : null);
        return {
            ...book,
