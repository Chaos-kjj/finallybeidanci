(function (root, factory) {
    ...
    function safeUrl(value, { allowImages = true, allowExternalLinks = true, allowExternalImages = false, allowInternalLinks = true, allowRelativeLinks = false, allowBlob = true, allowAppDict = true } = {}) {
        const raw = String(value || '').trim();
        if (!raw) return '';
        if (allowBlob && /^blob:/i.test(raw)) return raw;
        if (allowAppDict && /^app-dict:/i.test(raw)) return raw;
        if (allowImages && /^data:image\/(png|jpeg|gif|webp);base64,/i.test(raw) && raw.length < 2_000_000) return raw;
        if ((allowExternalLinks || (allowImages && allowExternalImages)) && /^(https?:|mailto:)/i.test(raw)) return raw;
        if (allowInternalLinks && /^#/i.test(raw)) return raw;
        if (allowRelativeLinks && !/^(?:[a-z][a-z0-9+.-]*:|\/\/|\\)/i.test(raw)) return raw;
        return '';
    }
        ...
        if (typeof DOMParser === 'undefined' || typeof document === 'undefined') {
            return source
                ...
                .replace(/\s(?:src|href)\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi, match => {
                    const attribute = match.match(/^\s*(src|href)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/i);
                    const value = attribute?.[2] ?? attribute?.[3] ?? attribute?.[4] ?? '';
                    return safeUrl(value, {
                        allowImages: String(attribute?.[1] || '').toLowerCase() === 'src',
                        allowExternalLinks: String(attribute?.[1] || '').toLowerCase() === 'href' && options.allowExternalLinks !== false,
                        allowExternalImages: options.allowExternalImages === true,
                        allowInternalLinks: options.allowInternalLinks !== false,
                        allowRelativeLinks: options.allowRelativeLinks === true,
                        allowBlob: options.allowBlob !== false,
                        allowAppDict: options.allowAppDict !== false
                    }) ? match : '';
                })
            ...
            Array.from(element.attributes).forEach(attribute => {
                ...
                if (name === 'href' || name === 'src') {
                    const url = safeUrl(attribute.value, {
                        allowImages: name === 'src',
                        allowExternalLinks: name === 'href' && options.allowExternalLinks !== false,
                        allowExternalImages: options.allowExternalImages === true,
                        allowInternalLinks: options.allowInternalLinks !== false,
                        allowRelativeLinks: options.allowRelativeLinks === true,
                        allowBlob: options.allowBlob !== false,
