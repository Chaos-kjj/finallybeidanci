(function (root, factory) {
    ...
    function copyIncrementalFields(overrides = {}) {
        const output = {};
        if (overrides.fsrsShadow && typeof overrides.fsrsShadow === 'object' && !Array.isArray(overrides.fsrsShadow)) {
            try { output.fsrsShadow = JSON.parse(JSON.stringify(overrides.fsrsShadow)); } catch (_) { /* ignore malformed optional data */ }
        }
        if (Array.isArray(overrides.reviewHistory)) {
            try { output.reviewHistory = JSON.parse(JSON.stringify(overrides.reviewHistory)); } catch (_) { output.reviewHistory = []; }
        }
        return output;
    }
    ...
    function createWordRecord(word, overrides = {}) {
        ...
        return {
            ...
            ...copyIncrementalFields(overrides)
        };
    }
    ...
    function buildQueue(words, settings, progress = { newWords: [], reviewWords: [] }, now = Date.now()) {
        ...
        const plan = normalizePlanProgress(progress);
