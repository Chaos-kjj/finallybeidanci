(function (root, factory) {
    ...
    class LocalStore {
        ...
        async migrateReaderSchema() {
            ...
            for (const book of books) {
                ...
                const notes = Array.isArray(book.notes) ? book.notes.map(item => normalizeReaderAnnotation(item, book, 'note')) : [];
                const bookmarks = Array.isArray(book.bookmarks) ? book.bookmarks.map(item => normalizeReaderAnnotation(item, book, 'bookmark')) : [];
                if (JSON.stringify(progress) !== JSON.stringify(book.progress || {}) || JSON.stringify(notes) !== JSON.stringify(book.notes || []) || JSON.stringify(bookmarks) !== JSON.stringify(book.bookmarks || [])) {
                    await this.put('books', { ...book, progress, notes, bookmarks, updatedAt: Number(book.updatedAt) || Date.now() });
                }
            }
            await this.put('meta', { id: 'reader-schema-v2', schemaVersion: DB_VERSION, completedAt: new Date().toISOString(), migratedBooks: books.length });
    ...
    function normalizeLegacyBook(book, legacyReaderProgress = null) {
        ...
        return {
            ...
            notes: Array.isArray(book.notes) ? book.notes.map(item => normalizeReaderAnnotation(item, book, 'note')) : [],
            bookmarks: Array.isArray(book.bookmarks) ? book.bookmarks.map(item => normalizeReaderAnnotation(item, book, 'bookmark')) : [],
            createdAt: Number(book.createdAt) || Date.now(),
            updatedAt: Number(book.updatedAt) || Date.now()
        };
    }

    function normalizeReaderAnnotation(annotation, book = {}, kind = 'note') {
        const source = annotation && typeof annotation === 'object' ? annotation : {};
        const anchor = source.anchor || source.selectionAnchor || source.positionAnchor || null;
        return {
            ...source,
            id: String(source.id || `${kind}-${Date.now()}-${Math.random().toString(36).slice(2)}`),
            kind: source.kind || kind,
            selectedText: String(source.selectedText || source.text || ''),
            content: String(source.content || ''),
            // Missing anchors are intentionally kept missing.  A legacy page
            // location is useful for display, but guessing a new text range
            // could silently attach a note to the wrong words.
            anchor: anchor || null,
            selectionAnchor: source.selectionAnchor || anchor || null,
            chapterIndex: Math.max(0, Number(source.chapterIndex) || 0),
            pageIndex: Math.max(0, Number(source.pageIndex) || 0),
            pdfPage: Math.max(1, Number(source.pdfPage) || 1),
            bookId: String(source.bookId || book.id || '')
        };
    }

    return { DB_NAME, DB_VERSION, STORES, LocalStore, MemoryBackend, upgradeDatabase, normalizeLegacyBook, normalizeReaderProgress, normalizeReaderAnnotation };
});
