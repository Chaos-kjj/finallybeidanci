(function (root, factory) {
    ...
    class LocalStore {
        ...
        async migrateLegacy({ legacyStorage = this.legacyStorage, legacyIndexedDB = globalThis.indexedDB } = {}) {
            ...
            const apiKey = String(oldAiConfig?.apiKey || oldAiConfig?.key || '');
            ...
            const existingSettings = await this.get('meta', 'settings');
            const legacyLastBookId = findLegacyLastBookId(oldReaderSettings, oldReaderProgress, oldState);
            const legacyReaderSettings = oldReaderSettings && typeof oldReaderSettings === 'object'
                ? (oldReaderSettings.reader && typeof oldReaderSettings.reader === 'object' ? oldReaderSettings.reader : oldReaderSettings)
                : {};
            if (Object.keys(legacyReaderSettings).length || legacyLastBookId) {
                await this.put('meta', {
                    id: 'settings',
                    data: {
                        ...(existingSettings?.data || {}),
                        reader: { ...(existingSettings?.data?.reader || {}), ...legacyReaderSettings },
                        ...(legacyLastBookId ? { lastBookId: legacyLastBookId } : {})
                    },
                    updatedAt: new Date().toISOString()
                });
            }
            const aiConfig = normalizeLegacyAiConfig(oldAiConfig);
            if (Object.keys(aiConfig).length) await this.put('meta', { id: 'ai-config', data: aiConfig });
            ...
            return { migrated: true, alreadyDone: false, apiKeyNeedsMigration, aiConfigMigrated: Boolean(Object.keys(aiConfig).length) };
        }
    }
    ...
    function normalizeLegacyBook(book, readerProgress = {}) {
        const id = String(book.id || `${Date.now()}-${Math.random().toString(36).slice(2)}`);
        const progressMap = readerProgress?.books || readerProgress || {};
        const legacyProgress = progressMap[id] || progressMap[`id:${id}`] || progressMap[book.fileName] || {};
        return {
            id,
            ...
            blob: book.blob || book.file || null,
            progress: { flow: 'scroll', chapterIndex: 0, pageIndex: 0, pdfPage: 1, percent: 0, ...(book.progress || {}), ...(legacyProgress || {}) },
    ...
    function normalizeLegacyAiConfig(raw) {
        if (!raw || typeof raw !== 'object') return {};
        const baseUrl = String(raw.baseUrl || raw.url || raw.endpoint || raw.apiUrl || '').trim();
        const model = String(raw.model || raw.modelName || '').trim();
        const config = {};
        if (baseUrl) config.baseUrl = baseUrl;
        if (model) config.model = model;
        if (raw.temperature !== undefined) config.temperature = Number(raw.temperature);
        if (raw.maxTokens !== undefined || raw.max_tokens !== undefined) config.maxTokens = Number(raw.maxTokens ?? raw.max_tokens);
        if (raw.timeoutMs !== undefined || raw.timeout !== undefined) config.timeoutMs = Number(raw.timeoutMs ?? raw.timeout);
        return config;
    }

    function findLegacyLastBookId(...sources) {
        for (const source of sources) {
            if (!source || typeof source !== 'object') continue;
            const candidates = [source.lastBookId, source.currentBookId, source.bookId, source.current?.bookId, source.current?.id];
    ...
    return { DB_NAME, DB_VERSION, STORES, LocalStore, MemoryBackend, upgradeDatabase, normalizeLegacyBook, normalizeLegacyAiConfig, findLegacyLastBookId };
});
