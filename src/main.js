...
function initNavigation() {
  ...
  $('home-continue-vocab')?.addEventListener('click', () => navigate('vocab'));
}
...
function addWordToErrataGroup(group, rawWord) {
  return addWordsToErrataGroup(group, [rawWord]);
}

async function addWordsToErrataGroup(group, words) {
  const merged = BookLibrary.mergeErrataWords({ errata: appState.errata, allWords: appState.allWords, group, words });
  if (!merged.group) return merged;
  appState.errata = merged.errata;
  appState.allWords = merged.allWords.map(item => item.word ? Learning.createWordRecord(item.word, item) : item);
  await persistState();
  updateWordSourceSelector();
  renderErrataManager();
  return merged;
...
function ensureReaderMenu() {
  ...
  $('reader-menu-toc').addEventListener('click', () => { hideReaderMenuPanels(['reader-toc']); show('reader-toc', true); });
  ...
  $('reader-menu-settings').addEventListener('click', () => {
    const settingsOpen = !$('reader-settings').classList.contains('hidden');
    hideReaderMenuPanels(settingsOpen ? [] : ['reader-settings']);
    if (!settingsOpen) show('reader-settings', true);
...
function hideReaderMenuPanels(keep = []) {
  document.querySelectorAll('#reader-menu-panels > *').forEach(panel => panel.classList.toggle('hidden', !keep.includes(panel.id)));
}

function toggleReaderMenu(visible = null) {
  const menu = ensureReaderMenu();
  const next = ReaderUi.toggleReaderMenu(menu, document.body, visible, $('reader-menu-title'), currentBook?.title || '阅读');
  if (!next) hideReaderMenuPanels([]);
  return next;
}
...
function initReader() {
  ...
  ['reader-font', 'reader-size', 'reader-weight', 'reader-letter', 'reader-line', 'reader-paragraph', 'reader-indent', 'reader-flow', 'reader-theme', 'reader-margin', 'reader-zoom', 'reader-fit', 'reader-rotate', 'reader-contrast', 'reader-crop', 'reader-invert', 'reader-grayscale'].forEach(id => $(id)?.addEventListener('input', scheduleReaderSettings));
  ['reader-flow', 'reader-theme', 'reader-fit', 'reader-rotate', 'reader-crop', 'reader-invert', 'reader-grayscale'].forEach(id => $(id)?.addEventListener('change', scheduleReaderSettings));
...
async function getBooks() {
  const rawBooks = await store.getAll('books');
  const books = rawBooks.map(book => BookLibrary.normalizeBookRecord(book));
  const sort = $('book-sort')?.value || 'recent';
  if (sort === 'title') return books.sort((left, right) => left.title.localeCompare(right.title, 'zh-Hans'));
...
function baseBookTitle(name) {
  return String(name || '未命名书籍').replace(/\.(?:epub|pdf|txt|md|html?|mht)$/i, '') || '未命名书籍';
}
...
async function inspectImportedBook(type, source, fallbackTitle) {
  if (type === 'epub') {
    ...
    return {
      ...
      coverDataUrl: parsed.cover?.data?.length ? bytesToDataUrl(parsed.cover.data, parsed.cover.mediaType) : ''
    };
  }
  if (type === 'html') return htmlBookMetadata(source, fallbackTitle);
  if (type === 'pdf') {
    const inspector = new Reader.PdfEngine(source, { title: fallbackTitle, pdfjsLib, disableWorker: true });
    try {
...
async function findDuplicateBook(fingerprint) {
  const books = await store.getAll('books');
  for (const raw of books) {
    const book = BookLibrary.normalizeBookRecord(raw);
    if (book.fingerprint && book.fingerprint === fingerprint) return book;
    let candidateBytes = null;
    if (book.blob?.arrayBuffer) candidateBytes = new Uint8Array(await book.blob.arrayBuffer());
    else if (book.text && ['text', 'html'].includes(book.type)) candidateBytes = new TextEncoder().encode(book.text);
    if (!candidateBytes?.length) continue;
    const existingFingerprint = await BookLibrary.fingerprintBytes(candidateBytes);
    if (existingFingerprint === fingerprint) {
      const migrated = { ...book, fingerprint: existingFingerprint, id: book.id || BookLibrary.stableBookId(existingFingerprint) };
...
function renderBookCard(book) {
  ...
  cover.src = book.coverDataUrl || BookLibrary.deterministicTextCover(book);
  ...
  body.append(title, author, meta, recent, actions);
...
function showBookDetails(book) {
  ...
  if (body) {
    ...
    [
      ['作者', book.author || '未注明'],
      ['格式', book.format || BookLibrary.formatLabel(book.type)],
      ['文件', book.fileName || '本地导入'],
      ['阅读进度', Math.round(book.progress?.percent || 0) + '%'],
      ['最近阅读', bookRecentLabel(book)]
    ].forEach(([label, value]) => { const row = document.createElement('p'); row.textContent = label + '：' + value; body.appendChild(row); });
  }
...
async function deleteBookById(id) {
  ...
  if (!book) return;
  if (typeof window.confirm === 'function' && !window.confirm('确定删除《' + book.title + '》吗？书籍文件、进度和书签都会移除。')) return;
  if (currentBook?.id === id) {
    await currentEngine?.close?.();
    currentBook = null;
    currentEngine = null;
    enterImmersiveReader(false);
  }
...
async function refreshBookList() {
  ...
  grid.innerHTML = '';
  if (!books.length) {
    const empty = document.createElement('p'); empty.className = 'muted book-grid-empty'; empty.textContent = '书架还是空的。点击“导入书籍”开始阅读。'; grid.appendChild(empty);
    return;
  }
...
async function openMostRecentBook() {
  const books = await getBooks();
  if (!books.length) { navigate('reader'); return; }
  navigate('reader');
...
async function importBook(file) {
  ...
  try {
    ...
    const duplicate = await findDuplicateBook(fingerprint);
    if (duplicate) {
      await refreshBookList();
    ...
    const source = type === 'text' || type === 'html' ? new TextDecoder().decode(sourceBytes) : sourceBytes.buffer;
    const text = type === 'html' ? Security.stripHtmlToText(source) : type === 'text' ? source : '';
    const fallbackTitle = baseBookTitle(file.name);
    const metadata = await inspectImportedBook(type, source, fallbackTitle);
    const now = Date.now();
    const book = BookLibrary.normalizeBookRecord({ id: BookLibrary.stableBookId(fingerprint), title: metadata.title || fallbackTitle, author: metadata.author || '', fileName: file.name, type, mime: file.type || 'application/octet-stream', size: sourceBytes.byteLength, fingerprint, coverDataUrl: metadata.coverDataUrl || BookLibrary.deterministicTextCover({ title: metadata.title || fallbackTitle, author: metadata.author || '', type }), blob: new Blob([sourceBytes], { type: file.type || 'application/octet-stream' }), text, progress: { chapterIndex: 0, pageIndex: 0, pageCount: 1, percent: 0, updatedAt: null }, notes: [], bookmarks: [], createdAt: now, updatedAt: now, lastReadAt: null });
...
async function openBook(id) {
  ...
  try {
    ...
    currentBook = BookLibrary.normalizeBookRecord({
      ...candidateBook,
      author: candidateBook.author || metadata.author || '',
      title: candidateBook.title || metadata.title || '未命名书籍'
    });
    currentBook = { ...currentBook, lastReadAt: currentBook.lastReadAt || new Date().toISOString(), updatedAt: Date.now() };
...
async function renderReader() {
  ...
  if (currentEngine instanceof Reader.PdfEngine && config.flow === 'pdf-original') {
    ...
    if (pdfjsLib.TextLayer && currentEngine.getTextContent) {
      ...
      try { const layer = new pdfjsLib.TextLayer({ textContentSource: await currentEngine.getTextContent(readerPdfPage), container: layerContainer, viewport: rendered.viewport }); await layer.render(); } catch (error) { console.warn('PDF 文本层不可用:', error.message || error); }
    }
    ...
    ReaderUi.applyReflowStyles(content, config);
...
function syncReaderSettingVisibility() {
  ...
  let flow = flowControl?.value || settings.reader?.flow || 'paged';
  if (hasBook) {
    const normalizedFlow = ReaderUi.normalizeFlow(format, flow);
    if (normalizedFlow !== flow) {
      flow = normalizedFlow;
      if (flowControl) flowControl.value = flow;
      if (settings.reader) settings.reader.flow = flow;
    }
  }
  ...
  document.querySelectorAll('[data-reader-setting="pdf-flow"]').forEach(element => element.classList.toggle('hidden', !visibility.pdfFlow));
  document.querySelectorAll('[data-reader-flow="pdf"]').forEach(option => {
    option.hidden = !visibility.pdfFlow;
    option.disabled = !visibility.pdfFlow;
  });
}
...
function scheduleReaderSettings() {
  readReaderSettings();
  syncReaderSettingVisibility();
  applyReaderTheme(settings.reader);
  if (currentEngine && !(currentEngine instanceof Reader.PdfEngine && settings.reader.flow === 'pdf-original')) ReaderUi.applyReflowStyles($('reader-content'), settings.reader);
  clearTimeout(readerRenderTimer);
  readerRenderTimer = setTimeout(() => { persistSettings(); renderReader(); }, 260);
...
function persistBookProgress() {
  ...
  const progress = { ...(currentBook.progress || {}), version: 2, chapterIndex: readerChapter, pageIndex: readerPage, pageCount: readerPageCount, pdfPage: readerPdfPage, location, percent: Math.round(Math.max(0, Math.min(1, fraction)) * 100), updatedAt: now };
  currentBook = { ...currentBook, progress, lastReadAt: now, updatedAt: Date.now() }; store.put('books', currentBook).catch(() => {}); renderPageLabel();
}
function advanceReader(delta) {
  ...
  readerPage += delta; setReaderPageOffset(readerPage); persistBookProgress(); renderPageLabel();
}
...
function resolveErrataPicker(result) {
  const resolver = errataPickerResolver;
...
function confirmErrataPicker() {
  ...
  const typedGroup = $('errata-picker-new-group')?.value.trim() || '';
  const group = typedGroup || errataPickerGroup || '';
  ...
  if (action.id === 'add-errata') {
    ...
    if (selection) {
      try {
        ...
        const duplicateText = result.duplicates.length ? '，跳过重复 ' + result.duplicates.length + ' 个' : '';
        setReaderNotice('已加入“' + result.group + '”：' + result.added.length + ' 个词' + duplicateText);
      } catch (error) {
...
async function addNoteFromCurrentSelection() { if (!currentBook || !readerSelection) return; const content = prompt(`为“${readerSelection.text}”添加笔记：`, '') || ''; if (!content.trim()) return; currentBook.notes ||= []; currentBook.notes.push({ id: `note-${Date.now()}`, selectedText: readerSelection.text, content: content.trim(), chapterIndex: readerChapter, pageIndex: readerPage, createdAt: new Date().toISOString() }); await store.put('books', { ...currentBook, updatedAt: Date.now() }); setReaderNotice('笔记已保存'); }

function renderAnnotationManager() {
  if (!currentBook) return;
  const menu = ensureReaderMenu();
  toggleReaderMenu(true);
  hideReaderMenuPanels(['reader-annotations-panel']);
  ...
  annotations.forEach(item => {
    ...
    const button = document.createElement('button'); button.textContent = `${item.kind} · ${item.label}`; button.addEventListener('click', async () => { readerChapter = Number(item.chapterIndex) || 0; readerPage = Number(item.pageIndex) || 0; readerPdfPage = Number(item.pdfPage) || 1; toggleReaderMenu(false); await renderReader(); });
  ...
  keyService = new Hardware.HardwareKeyService({
    ...
    onKey: (action, details, event) => {
      if (keyWizardAction && Number.isFinite(Number(details?.keyCode)) && Number(details.keyCode) >= 0) {
        ...
        settings.keyMapping = { ...(settings.keyMapping || Hardware.DEFAULT_MAPPING), [keyWizardAction]: [captured] };
        keyService?.setMapping(settings.keyMapping);
        const fieldId = { next: 'key-next', previous: 'key-previous', menu: 'key-menu' }[keyWizardAction];
        if (fieldId) $(fieldId).value = String(captured);
        const labels = { next: '下一页', previous: '上一页', menu: '菜单' };
        setText('key-wizard-status', '已设置“' + labels[keyWizardAction] + '”键：' + captured);
        keyWizardAction = null;
        persistSettings().catch(error => setText('key-wizard-status', '保存失败：' + error.message));
        return;
      }
      ...
      if (activePage === 'reader') {
        ...
        if (action === 'menu') {
          const isOpen = document.body.classList.contains('reader-menu-open');
          if (isOpen) toggleReaderMenu(false);
