...
(async () => {
  const epub = zipSync({
    ...
    'OEBPS/content.opf': new TextEncoder().encode('<package><metadata><dc:title>Test Book</dc:title><dc:creator>A. Author</dc:creator></metadata><manifest><item id="c1" href="chapter.xhtml" media-type="application/xhtml+xml"/><item id="nav" href="nav.xhtml" media-type="application/xhtml+xml" properties="nav"/><item id="cover" href="cover.jpg" media-type="image/jpeg" properties="cover-image"/></manifest><spine><itemref idref="c1"/></spine></package>'),
    ...
    'OEBPS/cover.jpg': new Uint8Array([255, 216, 255, 0])
  });
  const book = await parseEpubBook(epub);
  assert.equal(book.title, 'Test Book');
  assert.equal(book.author, 'A. Author');
  assert.equal(book.cover.mediaType, 'image/jpeg');
  assert.deepEqual([...book.cover.data], [255, 216, 255, 0]);
