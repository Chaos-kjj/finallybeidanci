...
settings.classList.toggle('hidden', true);
const toc = fakeElement();
toc.classList.toggle('hidden', true);
const close = fakeElement();
const settingsButton = fakeElement();
...
content.querySelectorAll = () => [paragraph];
...
assert.equal(content.style.fontFamily, 'monospace');
assert.equal(content.style.fontSize, '26px');
assert.equal(content.style.letterSpacing, '1.5px');
assert.equal(paragraph.style.marginBottom, '2em');
assert.equal(paragraph.style.textIndent, '1em');

const storage = { value: '', getItem() { return this.value; }, setItem(_key, value) { this.value = value; } };
const settingsStore = readerUi.createSettingsStorage(storage);
settingsStore.save({ fontSize: 29, theme: 'warm' });
...
(async () => {
  ...
  const cancelled = { selectedWords: [], group: '' };
  assert.equal(cancelled.selectedWords.length, 0, '取消选择不得写入错词本');
