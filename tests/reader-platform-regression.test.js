...
(async () => {
  ...
  const pdfEngine = await new PdfEngine(makePdf(), {
    pdfjsLib: pdfjs,
    disableWorker: true,
  ...
  const callerOwnedPdf = makePdf().buffer;
  const callerSnapshot = Buffer.from(new Uint8Array(callerOwnedPdf));
  const workerAttempts = [];
  const fakePage = {
    async getTextContent() { return { items: [{ str: 'Worker transfer copy' }] }; },
    cleanup() {}
  };
  const fakePdf = {
    numPages: 1,
    async getPage() { return fakePage; },
    async getMetadata() { return { info: {} }; },
    destroy() {}
  };
  const transferringPdfjs = {
    getDocument({ data, disableWorker }) {
      workerAttempts.push({ disableWorker, byteLength: data.byteLength });
      structuredClone(data.buffer, { transfer: [data.buffer] });
      return { promise: disableWorker ? Promise.resolve(fakePdf) : Promise.reject(new Error('worker unavailable')) };
    }
  };
  const transferEngine = await new PdfEngine(callerOwnedPdf, { pdfjsLib: transferringPdfjs }).open();
  assert.deepEqual(workerAttempts, [
    { disableWorker: false, byteLength: callerSnapshot.byteLength },
    { disableWorker: true, byteLength: callerSnapshot.byteLength }
  ], 'PDF worker fallback must receive a fresh, non-detached byte copy on every attempt');
  assert.equal(callerOwnedPdf.byteLength, callerSnapshot.byteLength, 'PDF.js must never detach the importer-owned ArrayBuffer');
  assert.deepEqual(Buffer.from(new Uint8Array(callerOwnedPdf)), callerSnapshot, 'PDF inspection must leave source bytes intact for Blob/IndexedDB storage');
  await transferEngine.close();
