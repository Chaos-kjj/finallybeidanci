...
['vocab-view-learn', 'vocab-view-challenge', 'vocab-view-translation', 'vocab-view-errata', 'vocab-view-lists', 'vocab-view-stats', 'learning-mode-select', 'known-words-list', 'review-words-list', 'word-ai-definition', 'home-calendar-grid', 'home-calendar-month'].forEach(id => assert(index.includes(`id="${id}"`), `背词/统计 UI 缺少 ${id}`));
['startLearningSession', 'handleKnownWord', 'handleUnknownWord', 'handleWordCorrection', 'finishWordReview', 'startSentenceChallenge', 'startTranslationChallenge', 'lookupWordWithAi', 'renderVocabStats', 'renderHomeCalendar'].forEach(name => assert(main.includes(`function ${name}`), `背词/统计逻辑缺少 ${name}`));
